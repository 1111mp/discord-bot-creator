...
import { explodePromise } from './explode-promise';
...
export function suspendTasksWithTimeout(): void {
  console.info(`TaskWithTimeout: suspending ${tasks.size} tasks`);
...
export function resumeTasksWithTimeout(): void {
  console.info(`TaskWithTimeout: resuming ${tasks.size} tasks`);
  ...
  return async (...args: Args) => {
    ...
    const startTimer = () => {
      ...
      timer = setTimeout(() => {
        ...
        console.error(toLogFormat(timeoutError));
        reject(timeoutError);
      }, timeout);
    };
    ...
    const entry: TaskType = {
      ...
      startedAt: undefined,
      suspend: () => {
        console.warn(`TaskWithTimeout: ${id} task suspended`);
        stopTimer();
      },
      resume: () => {
        console.warn(`TaskWithTimeout: ${id} task resumed`);
        startTimer();
      },
    };
